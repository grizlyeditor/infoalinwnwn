...
# Login URLs for different regions
LOGIN_URLS = {
    "IND": "https://loginind.ggblueshark.com/MajorLogin",
    "OTHER": "https://loginbp.ggblueshark.com/MajorLogin"
}
...
def get_account_credentials(region: str) -> str:
    r = region.upper()
    credentials_map = {
        "ME": "uid=4225983000&password=BY_GRIZLY-XGZZVNRRL-GRIZLY",
        "BD": "uid=4253807582&password=BY_GRIZLY-YDDBLLUUJ-GRIZLY", 
        "PK": "uid=4253803099&password=BY_GRIZLY-BEAT8UR22-GRIZLY",
        "IND": "uid=4253797944&password=BY_GRIZLY-CCHPODJBA-GRIZLY",
        "BR": "uid=4225984458&password=BY_GRIZLY-K81NT5FOD-GRIZLY",
        "US": "uid=4225984458&password=BY_GRIZLY-K81NT5FOD-GRIZLY",
        "SAC": "uid=4225984458&password=BY_GRIZLY-K81NT5FOD-GRIZLY",
        "ID": "uid=4253812230&password=BY_GRIZLY-XEZBWAOW7-GRIZLY",
        "VN": "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY",
        "TH": "uid=4219152882&password=BY_GRIZLY-QNXUYS3PG-GRIZLY",
        "CIS": "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY",
        "SG": "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY",
        "NA": "uid=4223599681&password=BY_GRIZLY-J8FJGJSVV-GRIZLY",
        "EU": "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY",
        "TW": "uid=4219152882&password=BY_GRIZLY-QNXUYS3PG-GRIZLY",
        "RU": "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY"
    }
    return credentials_map.get(r, "uid=4225982283&password=BY_GRIZLY-Y8FHHVJ39-GRIZLY")
...
async def create_jwt(region: str):
    try:
        ...
        proto_bytes = await json_to_proto(body, login_req)
        payload = aes_cbc_encrypt(MAIN_KEY, MAIN_IV, proto_bytes)
        
        # Region-specific login URL
        login_url = LOGIN_URLS["IND"] if region.upper() == "IND" else LOGIN_URLS["OTHER"]
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Create LoginRes instance and decode
            login_res = FreeFire_pb2.LoginRes()
            decoded_response = decode_protobuf(resp.content, FreeFire_pb2.LoginRes)
            msg = json.loads(json_format.MessageToJson(decoded_response))
            
            # Use region-specific client URL from server response or fallback
            server_url = msg.get('serverUrl', '0')
            if server_url == '0' or 'ggblueshark.com' in server_url:
                # Use our predefined region URLs
                server_url = REGION_URLS.get(region.upper(), "https://clientbp.ggblueshark.com/")
...
async def GetAccountInformation(uid, unk, region, endpoint):
    try:
        get_player_show = main_pb2.GetPlayerPersonalShow()
        payload = await json_to_proto(json.dumps({'a': uid, 'b': unk}), get_player_show)
        ...
        async with httpx.AsyncClient() as client:
            ...
            decoded_data = decode_protobuf(resp.content, AccountPersonalShow_pb2.AccountPersonalShowInfo)
            return json.loads(json_format.MessageToJson(decoded_data))
    except Exception as e:
...
@app.route('/')
def home():
    return jsonify({"message": "FreeFire Account Info API", "status": "active"})
...
async def get_account_info():
    ...
    if region not in SUPPORTED_REGIONS:
        return jsonify({"error": f"Region {region} not supported. Supported regions: {', '.join(SUPPORTED_REGIONS)}"}), 400
...
def get_supported_regions():
    return jsonify({"supported_regions": list(SUPPORTED_REGIONS)}), 200

@app.route('/region_urls')
def get_region_urls():
    return jsonify({"region_urls": REGION_URLS}), 200
